...
SELECT
    ...
    t.nome AS time
FROM jogador j
FULL OUTER JOIN JogadorTimeRelacao jt ON j.id = jt.jogador_id
FULL OUTER JOIN time t ON jt.time_id = t.id;
...
SELECT
    ...
    -- Calcula o K/D Ratio, tratando a divisão por zero (se um jogador tiver 0 mortes)
    CASE
        WHEN COALESCE(SUM(p.deaths), 0) = 0 THEN COALESCE(SUM(p.kills), 0)::numeric
        ELSE ROUND(COALESCE(SUM(p.kills), 0)::numeric / SUM(p.deaths)::numeric, 2)
    END AS kd_ratio,
